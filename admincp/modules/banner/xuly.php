<?php
	include("../connect.php"); //Kết nối đến CSDL
	mysql_query("SET NAMES utf8");
	//Lấy dữ liệu
	if ($_POST['thembanner']!="")
	{
		$tenalbum=$_POST['ten'];
		$name=$_FILES['anhminhhoa']['name'];//Lấy tên của tệp tin ảnh
		//echo $name;
		if(isset($name))
		{	
			$time=date("Ymdhis");
			$name=$time."_".$name;//Nối thêm ngày và thời gian vào tên file ảnh
			$dich="../../../images/uploads/".$name;// đường dẫn file ảnh 
			copy($_FILES['anhminhhoa']['tmp_name'],$dich); //Sao chép file ảnh đến đường dẫn đích
			$dich=substr($dich,9);//Cắt bớt 9 ký tự đầu trong đường dẫn
		}
		$trangthai=$_POST['trangthai'];
		$sql="INSERT INTO banner(ten,anhminhhoa,trangthai) VALUES ('$ten','$dich','$trangthai')";
		mysql_query($sql);
		header("location:../../index.php?ac=banner");		
	}	
	elseif($_POST['suabanner']!="")
	{
		//Thực hiện sửa loại tin
		$tenalbum=$_POST['ten'];
		$name=$_FILES['anhminhhoa']['name'];//Lấy tên của tệp tin ảnh
		if($name!='')
		{
			$time=date("Ymdhis");
			$name=$time."_".$name;//Nối thêm ngày và thời gian vào tên file ảnh
			$dich="../../../images/uploads/".$name;// đường dẫn file ảnh 
			copy($_FILES['anhminhhoa']['tmp_name'],$dich); //Sao chép file ảnh đến đường dẫn đích
			$dich=substr($dich,9);//Cắt bớt 9 ký tự đầu trong đường dẫn
		}
		$trangthai=$_POST['trangthai'];
		$idtheloai=$_POST['idtheloai'];
		//Định nghĩa câu lệnh sửa
		$sql="UPDATE banner SET ten='$ten', anhminhhoa='$dich',trangthai='$trangthai' WHERE idbanner='$_GET[id]'";
		//Thực hiện câu lệnh đã định nghĩa
		mysql_query($sql);
		header("location: ../../index.php?sua=banner&id=$_GET[id]");
	}
	else
	{
		//Thực hiện xóa bài viết
		$sql="delete from banner where idbanner='$_GET[id]'";
		mysql_query($sql);
		header("location: ../../index.php?ac=banner");
	}
?>